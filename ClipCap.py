...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key_value: torch.Tensor = None, mask: torch.Tensor = None) -> torch.Tensor:
        ...
        queries = self.to_queries(query).reshape(b, n, self.num_heads,
                                                 self.head_size)  # (batch_size, n_seq, num_heads, head_size)
        keys_values = self.to_keys_values(key_value).reshape(b, m, 2, self.num_heads,
                                                             self.head_size)  # (batch_size, m_seq, 2, num_heads, head_size)
        keys, values = keys_values[:, :, 0], keys_values[:, :,
                                             1]  # (batch_size, m_seq, num_heads, head_size), (batch_size, m_seq, num_heads, head_size)
        ...
        outputs = torch.einsum('bnmh,bmhd->bnhd', attention, values).reshape(b, n,
                                                                             d_query)  # (batch_size, n_seq, d_query)
...
class MappingNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        prefix = self.prefix_const.unsqueeze(dim=0).expand(x.shape[0],
                                                           *self.prefix_const.shape)  # (b, prefix_length, d_model)
...
def get_language_mode(lm_type):
    if 'gpt' in lm_type:
        ...
        raise NotImplementedError("{} not found".format(lm_type))
...
class ClipCaptionModel(nn.Module):
    ...
    def __init__(
        ...
        n_parameters = sum(p.numel() for p in self.gpt.parameters())
        print('number of self.gpt all params:', n_parameters, f'{n_parameters / 1e6:.3f}M')
        self.mapping_network = MappingNetwork(clip_project_length, clip_hidden_size, continuous_length,
                                              self.gpt_hidden_size, num_layers, num_heads)
    ...
    def word_embed(self, caption_tokens):
        if 'gpt' in self.gpt_type:
            ...
            caption_embeddings = self.gpt.get_input_embeddings()(caption_tokens)

        return caption_embeddings

    def get_dummy_token(self, batch_size: int, device: torch.device) -> torch.Tensor:
        return torch.zeros(batch_size, self.continuous_length, dtype=torch.int64, device=device)
    ...
    def forward(
        ...
        continuous_embeddings = self.mapping_network(continuous_prompt).view(-1, self.continuous_length,
                                                                             self.gpt_hidden_size)  # (b, continuous_length, gpt_hidden_size)
        if hard_prompts_length is not None:  # with hard prompts
            if self.only_hard_prompt:
                ...
                for i in range(len(hard_prompts_length)):
                    ...
                    temp_embeddings = torch.cat(
                        (caption_embeddings[i][:length], continuous_embeddings[i], caption_embeddings[i][length:]),
            ...
            embeddings = torch.cat((continuous_embeddings, caption_embeddings),
                                   dim=1)  # (b, continuous_length + caption_length, gpt_hidden_size)

        dummy_token = self.get_dummy_token(caption_tokens.shape[0], caption_tokens.device)
        labels = torch.cat((dummy_token, caption_tokens), dim=1)

        if 'gpt' in self.gpt_type:
            out = self.gpt(inputs_embeds=embeddings.type(self.gpt.dtype), attention_mask=mask)
        else:
            out = self.gpt(inputs_embeds=embeddings.type(self.gpt.dtype), labels=labels, attention_mask=mask)
